...
import { Phone, Users, Calendar, AlertCircle, Filter, Search, UploadCloud, Plus, Globe, MessageCircle, ThumbsUp, Camera, Mail, UserPlus, Store, PhoneCall } from 'lucide-react';
...
export default function LeadPoolPage() {
  ...
  const getSourceIcon = (source: string) => {
    ...
    if (s.includes('whatsapp')) return <MessageCircle className="w-4 h-4 text-green-500" />;
    if (s.includes('facebook')) return <ThumbsUp className="w-4 h-4 text-blue-500" />;
    if (s.includes('instagram')) return <Camera className="w-4 h-4 text-pink-500" />;
    if (s.includes('email')) return <Mail className="w-4 h-4 text-red-400" />;
    if (s.includes('referral')) return <UserPlus className="w-4 h-4 text-purple-400" />;
    if (s.includes('walk-in') || s.includes('walkin')) return <Store className="w-4 h-4 text-yellow-500" />;
    if (s.includes('phone') || s.includes('call')) return <PhoneCall className="w-4 h-4 text-blue-400" />;
