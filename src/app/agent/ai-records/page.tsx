  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        const res = await fetch(`${typeof window !== "undefined" ? (window.location.hostname.includes("devtunnels.ms") ? "https://" + window.location.hostname.replace("3000", "4000") : "http://" + window.location.hostname + ":4000") : "http://localhost:4000"}/api/v1/call-records`, {
          headers: {
            'x-user-role': role,
            'x-agent-id': agentId
          }
